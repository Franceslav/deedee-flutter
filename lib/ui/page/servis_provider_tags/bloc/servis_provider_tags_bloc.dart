...
part 'servis_provider_tags_event.dart';
part 'servis_provider_tags_state.dart';

class ProviderTagsBloc extends Bloc<ProviderTagsEvent, ProviderTagsState> {
  final TagRepository _tagRepository;
  final User _user;

  ProviderTagsBloc(this._tagRepository, this._user) : super(InitialState()) {
    on<DeleteTagEvent>(_onDeleteTags);
    on<SearchUserTagsEvent>(_onSearchTags);
    _init();
  }

  _onDeleteTags(DeleteTagEvent event, Emitter<ProviderTagsState> emit) async {
    try {
      final response =
  ...
  _onSearchTags(
      SearchUserTagsEvent event, Emitter<ProviderTagsState> emit) async {
